...
function getEthereumInfo(verify) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            console.log("Dir", __dirname);
            const transport = yield TransportNodeHid.open("");
            listen(log => console.log(log));
            //   console.error("transport", transport)
            const appEth = new AppEth(transport);
            const config = yield appEth.getAppConfiguration();
            console.log(config);
            const address = yield appEth.getAddress("44'/60'/0'/0/0", true, false);
            return address;
        }
        catch (e) {
            console.warn(e);
            // try again until success!
            return new Promise(s => setTimeout(s, 1000)).then(() => getEthereumInfo(verify));
        }
        ;
    });
}
